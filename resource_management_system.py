# -*- coding: utf-8 -*-
"""Resource Management System.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1kU03Wlf4DfelPJQmR-HY8lVw9WMu4oMy
"""
    ...
    def schedule(self):
        while self.ready_queue:
            ...
            if self.resource_manager.allocate(process):
                ...
                print(f"{process.name} is running for {run_time} seconds.")
                time.sleep(run_time)
                process.remaining_time -= run_time

                # Release resources and check if process is completed
...
"""Resource Manager with Deadlock Detection and Recovery"""
...
class ResourceManager:
    ...
    def request_additional_resources(self, process, additional_cpu, additional_memory):
        with self.lock:
            if self.available_cpu >= additional_cpu and self.available_memory >= additional_memory:
                self.available_cpu -= additional_cpu
                self.available_memory -= additional_memory
                process.cpu_needed += additional_cpu
                process.memory_needed += additional_memory
                print(f"Granted additional resources to {process.name}: {additional_cpu}% CPU, {additional_memory}MB memory.")
                return True
            print(f"Insufficient resources for {process.name} to request additional {additional_cpu}% CPU and {additional_memory}MB memory.")
            return False

    def detect_deadlock(self):
        with self.lock:
            if self.available_cpu == 0 and self.available_memory == 0:
    ...
    def recover_from_deadlock(self):
        with self.lock:
            if self.processes:
                process_to_terminate = sorted(self.processes, key=lambda p: p.priority)[0]
                print(f"Terminating {process_to_terminate.name} to recover from deadlock.")
                self.release(process_to_terminate)
...
class Process:
    def __init__(self, name, cpu_needed, memory_needed, priority):
        self.name = name
        self.cpu_needed = cpu_needed
        self.memory_needed = memory_needed
    ...
    def schedule(self):
        while self.ready_queue:
            ...
            if self.resource_manager.allocate(process):
                # Simulate running process
                run_time = min(process.remaining_time, process.time_slice)
                print(f"{process.name} (Priority {process.priority}) is running for {run_time} seconds.")
                time.sleep(run_time)
                process.remaining_time -= run_time
                ...
                if process.remaining_time > 0:
                    print(f"{process.name} is preempted with {process.remaining_time} seconds remaining.")
                    self.add_process(process)  # Re-add to queue
                else:
                    print(f"{process.name} has completed execution.")
            else:
                print(f"Insufficient resources for {process.name}, adding back to the queue.")
...
if __name__ == "__main__":
    ...
    def add_random_processes():
        while True:
            ...
            cpu_needed = random.randint(10, 30)
            memory_needed = random.randint(100, 500)
            priority = random.randint(1, 10)  # Higher numbers indicate higher priority
            process = Process(name, cpu_needed, memory_needed, priority)
            scheduler.add_process(process)
            print(f"Added {process.name} needing {process.cpu_needed}% CPU and {process.memory_needed}MB memory (Priority {process.priority}).")
            time.sleep(random.randint(3, 7))

    monitoring_thread = Thread(target=scheduler.monitor_resources)
    monitoring_thread.daemon = True
    monitoring_thread.start()

    process_addition_thread = Thread(target=add_random_processes)
    process_addition_thread.daemon = True
    process_addition_thread.start()

    scheduler.schedule()

"""With RR, FCFS, SRTF Comparison"""
